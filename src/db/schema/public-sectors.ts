// Public Sectors table - similar structure to businesses but without SKU
import { sqliteTable, text, integer, real, index } from 'drizzle-orm/sqlite-core';
import { sql } from 'drizzle-orm';

export const publicSectors = sqliteTable('public_sectors', {
  id: text('id').primaryKey(),
  // For compatibility with remote schema
  entityType: text('entity_type').default('public_sector'),
  ...
  ownerIdx: index('ps_owner_idx').on(table.ownerId),
  statusIdx: index('ps_status_idx').on(table.status),
  categoryIdx: index('ps_category_idx').on(table.categoryId),
  subscriptionStatusIdx: index('ps_subscription_status_idx').on(table.subscriptionStatus),
}));
