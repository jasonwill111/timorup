...
async function runFullTest() {
  ...
  try {
    ...
    if (currentUrl.includes('/account') || currentUrl.includes('success')) {
      log(`✓ Registration successful, redirected to: ${currentUrl}`);
    } else {
      log(`⚠ Registration redirect: ${currentUrl}`);
    }
    ...
    if (!page.url().includes('/login')) {
      log(`✓ User is logged in, on account page`);
    }
    ...
    if (logoutBtn) {
      ...
      log(`✓ Logged out, now at: ${page.url()}`);
    } else {
