---
id: US-002
feature: FS-039
title: "Business Actions Migration (P1)"
status: completed
priority: P1
created: 2026-05-09T00:00:00.000Z
tldr: "**As a** developer."
project: TimorLink
---

# US-002: Business Actions Migration (P1)

**Feature**: [FS-039](./FEATURE.md)

**As a** developer
**I want** business listing endpoints migrated to Server Actions
**So that** create/update/like operations use type-safe actions

---

## Acceptance Criteria

- [x] **AC-US2-01**: createBusiness action validates and creates listing âœ?- [x] **AC-US2-02**: updateBusiness action updates existing listing âœ?- [x] **AC-US2-03**: addUpdate action appends update to listing âœ?- [x] **AC-US2-04**: likeBusiness action toggles like status âœ?
---

## Implementation

**Increment**: [0039-server-actions-migration](../../../../../increments/0039-server-actions-migration/spec.md)

**Tasks**: See increment tasks.md for implementation details.


## Tasks

_No tasks defined for this user story_

