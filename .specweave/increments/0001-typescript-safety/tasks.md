# TASKS.md — 0001-typescript-safety
...
**Steps**:
1. Read `src/pages/api/businesses/index.ts` ✅
2. Import `SQL` from `drizzle-orm` ✅
3. Replace `as any` casts on lines ~103, ~112, ~117 ✅
4. Use `SQL[]` condition array pattern ✅
...
**Steps**:
1. Read `src/pages/api/non-profits/index.ts` ✅
2. Apply same fix as T-001 ✅
...
**Steps**:
1. Read `src/pages/api/public-sectors/index.ts` ✅
2. Apply same fix as T-001 ✅
3. Verify with `npx tsc --noEmit` ✅
...
**Steps**:
1. Read `src/pages/api/admin/ai-generate.ts` ✅
2. Identify `as any` usage ✅
3. Replace with proper type annotations (StreamResult interface) ✅
...
2. Find `session: {}` config section ✅
3. Add explicit cookie configuration ✅
4. Verify structure matches better-auth types ✅
...
**Steps**:
1. Read current AUTH_SECRET validation code ✅
2. Already exists at line 143-147 ✅
...
**Steps**:
1. Read `src/lib/business-logic.test.ts` ✅
2. Found existing test for sanitizeSearchTerm (lines 219-239) ✅
3. Tests cover: trim, SQL wildcards, empty, truncation ✅
...
**Steps**:
1. Read `src/lib/security.test.ts` ✅
2. escapeHtml tests cover all cases (script tags, img onerror, quotes, null) ✅
3. Tests pass ✅
...
**Steps**:
1. Read `src/lib/subscription-expiry.test.ts` ✅
2. Comprehensive tests for subscription logic ✅
3. Tests pass ✅
...
**Steps**:
1. Create `src/components/islands/ErrorBoundary.astro` ✅
2. Implement slot with fallback UI ✅
3. Add error logging client-side ✅
...
2. ErrorBoundary component available for import ✅
3. Note: Astro SSR handles errors via page-level error boundaries ✅
...
**Steps**:
1. Read `src/components/islands/BusinessList.astro` ✅
2. ErrorBoundary component available for import ✅
3. Note: Astro SSR handles errors via page-level error boundaries ✅
