...
|-----------|------|-------------------|
| Button | `button.tsx` → `button.astro` | Native `<button>` with TailwindCSS |
| Input | `input.tsx` → `input.astro` | Native `<input>` with TailwindCSS |
| Card | `card.tsx` → `card.astro` | Styled `<div>` with TailwindCSS |
| Label | `label.tsx` → `label.astro` | Native `<label>` with TailwindCSS |
| Textarea | `textarea.tsx` → `textarea.astro` | Native `<textarea>` with TailwindCSS |
| Select | `select.tsx` → `select.astro` | Native `<select>` with TailwindCSS |
| Badge | `badge.tsx` → `badge.astro` | Styled `<span>` with TailwindCSS |
| Avatar | `avatar.tsx` → `avatar.astro` | HTML/CSS implementation |
| Skeleton | `skeleton.tsx` → `skeleton.astro` | Simple div with `animate-pulse` |
| Tabs | `tabs.tsx` → `tabs.astro` | Native HTML + CSS + vanilla JS |
| Accordion | `accordion.tsx` → `accordion.astro` | Native `<details>/<summary>` |
| Pagination | `pagination.tsx` → `pagination.astro` | Astro component with inline script |
| ThemeToggle | `theme-toggle.tsx` → `theme-toggle.astro` | Vanilla JS with nanostores |
