...
import { C as CredentialsProviderError, I as chain, s as setCredentialFeature, j as getProfileName, J as readFile, H as HttpRequest } from "./media_rgSTBrWs.mjs";
import { createHash, createPrivateKey, createPublicKey, sign } from "node:crypto";
import { f as fs } from "./promises_Bc5ugn54.mjs";
import { G as homedir } from "./os_DXFRywg5.mjs";
import { dirname, join } from "node:path";
import { p as parseKnownFiles } from "./parseKnownFiles_BDrPfknD.mjs";
const resolveCredentialSource = (credentialSource, profileName, logger) => {
  const sourceProvidersMap = {
    EcsContainer: async (options) => {
      const { fromHttp } = await import("./index_DSNXw3BO.mjs");
      const { fromContainerMetadata } = await import("./index_DhQh9N_G.mjs");
      logger?.debug("@aws-sdk/credential-provider-ini - credential_source is EcsContainer");
      return async () => chain(fromHttp(options ?? {}), fromContainerMetadata(options))().then(setNamedProvider);
    },
    Ec2InstanceMetadata: async (options) => {
      logger?.debug("@aws-sdk/credential-provider-ini - credential_source is Ec2InstanceMetadata");
      const { fromInstanceMetadata } = await import("./index_DhQh9N_G.mjs");
      return async () => fromInstanceMetadata(options)().then(setNamedProvider);
    },
    Environment: async (options) => {
      logger?.debug("@aws-sdk/credential-provider-ini - credential_source is Environment");
      const { fromEnv } = await import("./index_C1mnLswQ.mjs");
...
const resolveAssumeRoleCredentials = async (profileName, profiles, options, callerClientConfig, visitedProfiles = {}, resolveProfileData2) => {
  ...
  if (!options.roleAssumer) {
    const { getDefaultRoleAssumer } = await import("./index_Buzrhats.mjs");
...
class LoginCredentialsFetcher {
  ...
  async refresh(token) {
    const { SigninClient, CreateOAuth2TokenCommand } = await import("./index_DzDNUD9C.mjs");
...
const resolveProcessCredentials = async (options, profile) => import("./index_CiCBp0bN.mjs").then(({ fromProcess }) => fromProcess({
  ...options,
  profile
...
const resolveWebIdentityCredentials = async (profile, options, callerClientConfig) => import("./index_bOUhIfq7.mjs").then(({ fromTokenFile }) => fromTokenFile({
