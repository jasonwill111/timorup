...
import { m as entityKind, X as TableName, e as ColumnBuilder, C as Column, o as getColumnNameAndConfig, U as textDecoder, s as sql, T as Table } from "./utils_CzyLOgOI.mjs";
...
const users = sqliteTable("users", {
  ...
  emailIdx: index("users_email_idx").on(users2.email)
}));
const categories = sqliteTable("categories", {
  ...
  // 'emoji:🍽️' or 'lucide:utensils' or ''
  parentId: text("parent_id"),
  entityType: text("entity_type").default("business"),
  // 'business' | 'government' | 'nonprofit' | null (all types)
  createdAt: integer("created_at", { mode: "timestamp" }).default(sql`(strftime('%s', 'now'))`),
  updatedAt: integer("updated_at", { mode: "timestamp" }).default(sql`(strftime('%s', 'now'))`)
});
...
const businessPages = sqliteTable("business_pages", {
  id: text("id").primaryKey(),
  title: text("title").notNull(),
  slug: text("slug").notNull().unique(),
  ...
  // Link to official registration
  verifiedBadge: integer("verified_badge", { mode: "boolean" }).default(false),
  // Social media links
  socialLinks: text("social_links"),
  // JSON: { facebook, instagram, tiktok }
  // Photo gallery (separate from banner/profile)
  photoGallery: text("photo_gallery"),
  // JSON array of media IDs (max 6 images + 1 video)
  // Latest update tracking (weekly limit)
  latestUpdate: text("latest_update"),
  // Text content
  latestUpdateImages: text("latest_update_images"),
  // JSON array of media IDs (max 3)
  latestUpdateDate: integer("latest_update_date", { mode: "timestamp" }),
  createdAt: integer("created_at", { mode: "timestamp" }).default(sql`(strftime('%s', 'now'))`),
  updatedAt: integer("updated_at", { mode: "timestamp" }).default(sql`(strftime('%s', 'now'))`)
}, (businessPages2) => ({
  ownerIdx: index("business_owner_idx").on(businessPages2.ownerId),
  statusIdx: index("business_status_idx").on(businessPages2.status),
  entityTypeIdx: index("business_entity_type_idx").on(businessPages2.entityType)
}));
const products = sqliteTable("products", {
  ...
  businessIdx: index("products_business_idx").on(products2.businessPageId)
}));
...
const reviews = sqliteTable("reviews", {
  ...
  businessIdx: index("reviews_business_idx").on(reviews2.businessPageId),
  userIdx: index("reviews_user_idx").on(reviews2.userId)
}));
const orders = sqliteTable("orders", {
  ...
  businessIdx: index("orders_business_idx").on(orders2.businessPageId),
  userIdx: index("orders_user_idx").on(orders2.userId)
}));
const adBanners = sqliteTable("ad_banners", {
  id: text("id").primaryKey(),
  title: text("title").notNull(),
  description: text("description"),
